...
{
	return (rbd_dev->header.features & RBD_FEATURE_FAST_DIFF &&
...
void __rbd_img_obj_request_submit(struct rbd_obj_request *obj_request)
{
	rbd_img_obj_request_submit(obj_request);
}

static int rbd_send_object_map_update(struct rbd_obj_request *obj_request)
{
	struct rbd_device *rbd_dev = obj_request->img_request->rbd_dev;
...
static int rbd_img_obj_request_submit(struct rbd_obj_request *obj_request)
{
	if (rbd_needs_object_map_update(obj_request))
		return rbd_send_object_map_update(obj_request);
...
{
	if (rbd_dev->header.image_size == 0)
		return rbd_object_map_bytes(0);

	return rbd_object_map_bytes(((rbd_dev->header.image_size - 1) >>
...
static int rbd_send_async_notify(struct rbd_device *rbd_dev,
				 struct page **reply,
				 size_t reply_len)
{
	struct ceph_osd_request *osd_req;
			      ...
			      int reply)
{
	...
	if (rbd_dev->lock_state == RBD_LOCK_STATE_LOCKED) {
		...
		p = reply_buf;
		ceph_encode_32(&p, sizeof(reply) +
			       CEPH_ENCODING_START_BLK_LEN);
		ceph_start_encoding(&p, 1, 1, sizeof(reply));
		ceph_encode_32(&p, reply);
	}

	ret = rbd_obj_notify_ack_sync(rbd_dev, notify_id, reply_buf,
				      reply_len);
			    ...
			    u32 payload_len)
{
	...
	ret = rbd_send_async_notify(rbd_dev, pages, alloc_len);

	ceph_release_page_vector(pages, page_count);
	return ret;
}

static int __notify_simple_op(struct rbd_device *rbd_dev,
			      enum rbd_notify_op op)
{
	void *reply;
	u32 message_len;
	void *p;
...
static int rbd_notify_acquired_lock(struct rbd_device *rbd_dev)
{
...
static int rbd_notify_released_lock(struct rbd_device *rbd_dev)
{
	BUG_ON(rbd_dev->lock_state == RBD_LOCK_STATE_LOCKED);
...
static int rbd_notify_request_lock(struct rbd_device *rbd_dev)
{
			    ...
			    struct rbd_spec *parent_spec)
{
	...
	dout("%s: remove from %s\n", __func__, rbd_dev->parent->header_name);
...
static void rbd_trim_remove_object(struct work_struct *work)
{
	...
	if (byte % step)  {
		trim_len = min_t(u64, (step - (byte % step)),
				 (rbd_dev->header.image_size - byte));
					 ...
					 void *start, void *end)
{
	...
	num_objects = rbd_dev->header.image_size >> rbd_dev->header.obj_order;
	...
	op = rbd_start_async_op(rbd_dev, 0, rbd_dev->header.image_size, gid,
				handle, request_id,
...
out:
	kref_put(&op->kref, __release_async_op);
	rbd_notify_async_complete(rbd_dev, gid, handle, request_id, ret);
	rbd_warn(rbd_dev, "object map rebuild complete: %d\n", ret);
	return ret;
err:
	return -ERANGE;
}
...
static int rbd_object_map_resize(struct rbd_device *rbd_dev, u64 size)
{
	struct page *req_data;
	size_t req_len;
	u64 object_count = (size >> rbd_dev->header.obj_order) + 1;
	u8 default_state = RBD_OBJECT_NONEXISTENT;
	u64 new_size = rbd_object_map_bytes(object_count);
	void *p;
	int i;
	int ret;
	...
	rbd_dev->object_map_data = krealloc(rbd_dev->object_map_data,
					    new_size, GFP_NOIO);
	...
	for (i = (rbd_dev->header.image_size >> rbd_dev->header.obj_order) + 1;
	     i < object_count;
	     i++)
		__rbd_object_map_set(rbd_dev, i, RBD_OBJECT_NONEXISTENT);
	up_write(&rbd_dev->object_map_rwsem);
...
static int rbd_object_map_snap_update(struct rbd_device *rbd_dev)
{
	...
	down_write(&rbd_dev->object_map_rwsem);
	for (i = 0;
	     i < rbd_dev->header.image_size >> rbd_dev->header.obj_order;
	     i++) {
...
static int rbd_lock(struct rbd_device *rbd_dev)
{
	...
	rbd_notify_acquired_lock(rbd_dev);
...
start:
	...
	if (ret < 0) {
		...
		ret = rbd_notify_request_lock(rbd_dev);
		if (ret == -ETIMEDOUT)
...
static int rbd_unlock(struct rbd_device *rbd_dev)
{
	...
	if (ret) {
		...
		rbd_dev->lock_state = RBD_LOCK_STATE_NOT_LOCKED;
		rbd_notify_released_lock(rbd_dev);
	}
...
static int rbd_object_map_load(struct rbd_device *rbd_dev)
{
	...
	object_map_size = rbd_object_map_size(rbd_dev);
	...
	if (resp_len != object_map_size) {
		dout("%s expecting size %llu got %ld\n",
