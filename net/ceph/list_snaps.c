...
static int __decode_clone(struct ceph_clone_info *out, void **p, void *end)
{
	...
	out->snaps = kmalloc(out->num_snaps * sizeof(u64), GFP_NOIO);
	if (!out->snaps) {
		ret = -ENOMEM;
		goto out;
	}
	...
	out->overlaps = kmalloc(out->num_overlaps *
				sizeof(struct ceph_clone_overlap), GFP_NOIO);
	if (!out->overlaps) {
		kfree(out->snaps);
		ret = -ENOMEM;
		goto out;
			   ...
			   struct ceph_snap_list *snaps)
{
	...
	snaps->clones = kmalloc(num * sizeof(struct ceph_clone_info), GFP_NOIO);
	if (!snaps->clones) {
		ret = -ENOMEM;
		goto out;
	}
