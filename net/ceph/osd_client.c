			      ...
			      struct ceph_msg *msg)
{
	...
	if (!osd_req->r_reply_op_len[which]) {
		dout("no notify_id received\n");
		goto out;
	}

	p = page_address(osd_data->pages[0]);
	end = p + osd_req->r_reply_op_len[which];
			      ...
			      u32 *num_watchers)
{
	...
	p = page_address(pg);
	end = p + osd_req->r_reply_op_len[0];
	...
	*watchers = kzalloc(*num_watchers * sizeof(**watchers), GFP_NOIO);
	if (!watchers) {
		ret = -ENOMEM;
		goto out;
	}
	...
	switch(opcode) {
		...
		case CEPH_WATCH_EVENT_DISCONNECT:
			if (event) {
				...
				event_work->payload = kmalloc(payload_len,
							      GFP_NOIO);
				memcpy(event_work->payload, payload,
				       payload_len);
...
int ceph_osdc_init(struct ceph_osd_client *osdc, struct ceph_client *client)
{
	...
	err = -ENOMEM;
	osdc->notify_wq = alloc_workqueue("ceph-watch-notify",
